...
# Thanks for viewing DASHOS Source Code! Leave any suggested improvements as I would love looking through them! Have a nice day :)

# Known errors:
# Cannot run gaminghub or any external python file twice
# Prints more file than it should when viewing python files
# Can input more than 1 letter but only counts as 1 hangman

# Roadmap:
# Logging everything (If practical as would drastically decrease performance) (import logging)
...
def Loginsuccessor():
  ...
  time.sleep(1.5)
  os.system('cls' if os.name == 'nt' else 'clear')
  print("Use 'help' to get a list of command and what they do")
...
def Error(protocol):
 ...
 |_|/_/   \_\_/_/   \_\_____| |_____|_| \_\_| \_\ \___/|_| \_\\
  ...
  # If LICENSE missing or corrupted, disallow access to DASHOS
  if protocol == "LICENSE_NOT_FOUND" or "LICENSE_INCORRECT":
    cprint("The LICENSE file, which contains the license you use <DASHOS> under, is either missing or corrupted\nYou must redownload this file before continuing to use <DASHOS>", "red")
    time.sleep(1)
    Shutdown()
...
def Cmdline(currentuser, currentdir):
  ...
  try:
    if len(currentcmd) < 2:
      cprint("Please enter a command", promptcolour)
      Cmdline(currentuser, currentdir)
    ...
    if currentcmd.lower() == "license":
      f = open("LICENSE", "r")
      license = f.read()
      cprint("\n" + license, "white", "on_grey", attrs=["bold"])
      Cmdline(currentuser, currentdir)
    ...
    elif currentcmd[0].lower() == "c" and currentcmd[1].lower() == "r" and currentcmd[2].lower() == "e" and currentcmd[3].lower() == "a" and currentcmd[4].lower() == "t" and currentcmd[5].lower() == "e":
      Createfile(currentcmd, currentdir)

    else:
      cprint("Command does not exist\nUse 'help' to get a list of commands and what they do", "white", attrs=["dark"])
      Cmdline(logincurrentuser, currentdir)
  except:
    cprint("An error occured interpreting your command\nIt was either entered incorrectly or you found a bug!\nPlease run 'help' to ensure the command was entered correctly", "red")
    Cmdline(currentuser, currentdir)
...
def Pythonfile(currentuser, currentdir):
  ...
  # If delete script
    ...
    # If trying to delete important file deny request
    if fname == "main.py" or fname == "gaminghub.py" or fname == "changelog" or fname == "LICENSE":
      cprint("Permission denied", "red")
...
def Help(currentuser, currentdir):
  ...
  HELP:
    COMMANDS:
        ...
        license: Shows you the license you use <DASHOS> under
...
def Createfile(currentcmd, currentdir):
  ...
  if filename == "numusers" or filename == "main.py" or filename == "config" or filename == "gaminghub.py" or filename == "changelog" or filename.upper() == "LICENSE":
    cprint("Permission denied", "red")
    Cmdline(logincurrentuser, currentdir)
  for i in range(0,10):
...
def Changedirectory(currentcmd, currentuser, currentdir):
  ...
  if directory[0] == "?" or directory == ".." or directory == ".":
    # If changing directory to current directory
    if directory == ".":
      Cmdline(currentuser, currentdir)
    # If using ? in cmd
    if directory[0] == "?":
...
try:
  ...
  line1 = f.readline()
  f.close()
  if line1 != "BSD 3-Clause License\n":
    Error("LICENSE_INCORRECT")
except IOError:
